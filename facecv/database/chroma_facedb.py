...
class ChromaFaceDB(AbstractFaceDB):
    ...
    def add_face(self, name: str, embedding: List[float], metadata: Optional[Dict] = None) -> str:
        ...
        # Add to ChromaDB
        self.collection.add(embeddings=[embedding], metadatas=[meta], ids=[face_id])
    ...
    def get_face(self, face_id: str) -> Optional[Dict]:
        """Get a face by ID"""
        try:
            result = self.collection.get(ids=[face_id], include=["embeddings", "metadatas"])
    ...
    def get_faces_by_name(self, name: str) -> List[Dict]:
        ...
        try:
            result = self.collection.get(where={"name": name}, include=["embeddings", "metadatas"])
    ...
    def search_similar_faces(
        self, embedding: List[float], threshold: float = 0.6, limit: int = 10
        ...
        try:
            ...
            results = self.collection.query(
                query_embeddings=[embedding], n_results=limit, include=["embeddings", "metadatas", "distances"]
            )
    ...
    def update_face(self, face_id: str, new_name: str, metadata: Optional[Dict] = None) -> bool:
        ...
        try:
            ...
            self.collection.update(ids=[face_id], metadatas=[update_metadata])
    ...
    def get_all_faces(self, limit: int = 1000) -> List[Dict]:
        ...
        try:
            result = self.collection.get(limit=limit, include=["embeddings", "metadatas"])
    ...
    def query_faces_by_embedding(
        self, embedding: np.ndarray, top_k: int = 10, threshold: float = 0.0
    ) -> List[Dict[str, Any]]:

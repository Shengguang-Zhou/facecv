...
def get_chromadb_facedb():
    ...
    if _chromadb_facedb is None:
        try:
            from .chroma_facedb import CHROMADB_AVAILABLE, ChromaFaceDB
...
class FaceDBFactory:
    ...
    @classmethod
    def create_database(self, db_type: Optional[str] = None, config=None, **kwargs) -> AbstractFaceDB:
        """
        创建数据库实例
...
def create_face_database(
    db_type: Optional[str] = None, connection_string: Optional[str] = None, **kwargs
) -> AbstractFaceDB:
    """
    创建人脸数据库实例的便捷函数

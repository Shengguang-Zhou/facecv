...
class VideoExtractor:
    ...
    def extract_single_frame(
        self, time_position: float, target_size: Optional[Tuple[int, int]] = None
    ) -> Optional[np.ndarray]:
        """
        提取指定时间位置的单帧
...
class VideoProcessor:
    ...
    ) -> Generator[Tuple[np.ndarray, Any, float], None, None]:
        """
        处理视频流
    ...
    def resize_video(
        self, input_path: str, output_path: str, target_size: Tuple[int, int], keep_aspect_ratio: bool = True
    ) -> bool:
        """
        调整视频尺寸

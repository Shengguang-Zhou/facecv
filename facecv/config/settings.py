...
class Settings(BaseSettings):
    ...
    deepface_distance_metric: Literal["cosine", "euclidean", "euclidean_l2"] = Field(
        default="cosine", description="距离度量方式 - cosine(推荐), euclidean(欧氏), euclidean_l2(标准化欧氏)"
    )
    ...
    secret_key: str = Field(
        default="facecv-default-secret-key-change-in-production", min_length=32, description="JWT密钥"
    )
    ...
    max_upload_size: int = Field(
        default=10 * 1024 * 1024, ge=1024, le=100 * 1024 * 1024, description="最大上传文件大小(字节)"
    )
    allowed_extensions: List[str] = Field(
        default=[".jpg", ".jpeg", ".png", ".bmp", ".webp"], description="允许的文件扩展名"
    )
    ...
    def get_arcface_model_path(
        self, backbone: Optional[str] = None, dataset: Optional[str] = None, filename: Optional[str] = None
    ) -> str:
        """获取ArcFace模型文件完整路径"""
...
def load_model_config(
    config_path: Optional[Union[str, Path]] = None, environment: str = "production"
) -> Dict[str, Any]:

...
from .stranger import StrangerDetector, AlertLevel
...
class VideoProcessor:
        ...
        try:
            ...
            if self.processing_mode == ProcessingMode.RECOGNITION_ONLY:
                ...
                if attendance_type and self.attendance_system:
                    attendance_result = self._process_attendance(
                        image, attendance_type, notes
                    )
                ...
                if self.stranger_detector:
                    stranger_result = self.stranger_detector.detect_stranger(
                        image, location, save_image
                    )
                    ...
                    # 触发陌生人警报回调
                    if (stranger_result.get('is_stranger') and 
                        stranger_result.get('alert_generated') and 
                        self.on_stranger_alert):
                        try:
                ...
                if attendance_type and self.attendance_system:
                    ...
                    # 触发考勤事件回调
                    if (attendance_result.get('success') and 
    ...
    def _process_attendance(
        ...
        if attendance_type == AttendanceType.CHECK_IN:
            ...
            return {
                'success': False,
                'message': f'不支持的考勤类型: {attendance_type}'
            }
    ...
    def get_processing_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        for mode in ProcessingMode:
            mode_stats[mode.value] = len([
                r for r in recent_results if r.mode == mode
            ])
        ...
        if self.attendance_system:
            ...
            for att_type in AttendanceType:
                attendance_stats['by_type'][att_type.value] = len([
                    ...
                    if r['attendance_type'] == att_type.value
                ])

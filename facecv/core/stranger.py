...
class StrangerDetector:
    ...
    def detect_stranger(
        self, image: np.ndarray, location: Optional[str] = None, save_image: bool = True
    ) -> Dict[str, Any]:
        """
        检测陌生人
        ...
        try:
            ...
            return {"is_stranger": False, "error": str(e), "timestamp": current_time.isoformat(), "location": location}
    ...
    def _should_generate_alert(self, location: Optional[str], current_time: datetime) -> bool:
        ...
        recent_alerts = [
            alert
            for alert in self.stranger_alerts
        ...
        hourly_alerts = [alert for alert in self.stranger_alerts if alert.timestamp > hour_threshold]
    ...
    def _generate_alert(
        ...
        alert = StrangerAlert(
            ...
            metadata={"recognition_confidence": confidence, "threshold_used": self.stranger_threshold},
        )
    ...
    def _record_detection(self, location: Optional[str], timestamp: datetime, is_stranger: bool, confidence: float):
        """
        记录检测结果
        ...
        self.recent_detections = [record for record in self.recent_detections if record["timestamp"] > hour_ago]
    ...
    def get_detection_statistics(self, hours: int = 24) -> Dict[str, Any]:
        ...
        # 统计检测记录
        recent_detections = [record for record in self.recent_detections if record["timestamp"] > time_threshold]

        # 统计警报
        recent_alerts = [alert for alert in self.stranger_alerts if alert.timestamp > time_threshold]
        ...
        for level in AlertLevel:
            alert_level_stats[level.value] = len([a for a in recent_alerts if a.alert_level == level])

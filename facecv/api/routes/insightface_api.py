...
from facecv.schemas.face import FaceDetection, FaceRegisterResponse, RecognitionResult, VerificationResult
...
def get_recognizer():
    ...
    arcface_enabled = runtime_config.get("arcface_enabled", getattr(settings, "arcface_enabled", False))
    ...
    if arcface_enabled:
        ...
        det_size = tuple(
            runtime_config.get("insightface_det_size", getattr(settings, "insightface_det_size", [640, 640]))
        )
        det_thresh = runtime_config.get("insightface_det_thresh", getattr(settings, "insightface_det_thresh", 0.5))
        similarity_threshold = runtime_config.get(
            "insightface_similarity_thresh", getattr(settings, "insightface_similarity_thresh", 0.35)
        )

        # Auto-select ArcFace model based on backbone preference
        backbone = runtime_config.get("arcface_backbone", getattr(settings, "arcface_backbone", "resnet50"))
        ...
        logger.info(
            f"  Dataset: {runtime_config.get('arcface_dataset', getattr(settings, 'arcface_dataset', 'webface600k'))}"
        )
        logger.info(
            f"  Embedding Size: {runtime_config.get('arcface_embedding_size', getattr(settings, 'arcface_embedding_size', 512))}"
        )
        ...
        model_pack = runtime_config.get(
            "insightface_model_pack", getattr(settings, "insightface_model_pack", "buffalo_l")
        )
        det_size = tuple(
            runtime_config.get("insightface_det_size", getattr(settings, "insightface_det_size", [640, 640]))
        )
        ...
        enable_emotion = runtime_config.get("enable_emotion", getattr(settings, "insightface_enable_emotion", True))
        enable_mask = runtime_config.get("enable_mask", getattr(settings, "insightface_enable_mask", True))
        prefer_gpu = runtime_config.get("prefer_gpu", getattr(settings, "insightface_prefer_gpu", True))
            ...
            for model_name in available_arcface:
                ...
                if model_info:
                    arcface_models[model_name] = {
                        ...
                        "recommended_use": (
                            "移动端、边缘计算" if model_info.backbone == "mobilefacenet" else "生产环境、高精度识别"
                        ),
                    }
...
async def list_faces(
    ...
    **返回:**
    ...
    try:
        ...
        return {"faces": clean_faces, "total": len(faces), "returned": len(limited_faces)}
...
async def delete_faces_by_name(name: str):
    ...
    **错误:**
    ...
    try:
        ...
        return {"message": f"Successfully deleted all faces for {name}", "deleted_count": deleted_count}
    ...
    try:
        ...
        return {
            ...
            "recommendations": {
                ...
                "gpu_recommendation": (
                    "建议使用GPU加速以获得更好性能"
                    if not gpu_available and getattr(recognizer, "prefer_gpu", True)
                    else "GPU配置正常"
                ),
                "model_recommendation": (
                    f"当前使用 {model_info.get('model_pack', 'buffalo_l')} - 适合生产环境"
                    if model_info.get("model_pack", "buffalo_l") == "buffalo_l"
                    else f"当前使用 {model_info.get('model_pack', 'buffalo_l')} - 考虑使用buffalo_l获得更高精度"
                ),
            },
        }
    ...
    try:
        ...
        status_info = {
            ...
            "loaded_models": (
                ...
                if recognizer.app and hasattr(recognizer.app, "models")
                else []
            ),
            "database_status": {
        ...
        return {"status": "unhealthy", "service": "InsightFace API", "error": str(e), "timestamp": str(datetime.now())}
...
async def get_video_sources():
    ...
    **返回:**
    ...
    for i in range(3):  # 检查前3个摄像头
        ...
        if cap.isOpened():
            cameras.append({"index": i, "name": f"摄像头 {i}", "available": True})
            cap.release()

    return {
        "cameras": cameras,
        "sample_rtsp": ["rtsp://username:password@192.168.1.100:554/stream1", "rtsp://192.168.1.100:8554/live.sdp"],
    }
